...
void disassemble_i_type(uint32_t instr, char *asm1)
{
    ...
    // 處理 `jalr` 指令
    if (opcode == 0x67 && funct3 == 0x0)
    { // opcode 為 1100111 且 funct3 為 000
        if (rd == 0 && imm == 0 && rs1 == 1)
        {
    ...
    { // opcode 為 1110011 且 funct3 為 000
        if (imm == 0)
        {
...
void disassemble(uint32_t instr, char *asm1)
{
    ...
    {
    case 'R':
        disassemble_r_type(instr, asm1);
        break;
    case 'I':
        disassemble_i_type(instr, asm1);
        break;
    case 'S':
        disassemble_s_type(instr, asm1);
        break;
    case 'B':
        disassemble_b_type(instr, asm1);
        break;
    case 'U':
        disassemble_u_type(instr, asm1);
        break;
    case 'J':
        disassemble_j_type(instr, asm1);
        break;
    default:
        sprintf(asm1, "?");
    }
