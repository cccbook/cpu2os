...
// 根據指令類型調用對應的反組譯函數
char do_instr(uint32_t instr, char *asm1)
{
    uint32_t opcode = instr & 0x7F;
    char type = '?';
    switch (opcode)
...
void do_block(char *block, int size)
{
    for (int pc = 0; pc < size; pc += 4)
    {
        ...
        char asm1[100];
        char type = disassemble(instruction, asm1);
