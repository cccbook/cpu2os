...
pub export fn int_sort(arg_array: [*c]c_int, arg_count: usize) void {
    ...
    {
        var i: c_int = 0;
        while (@as(usize, @bitCast(@as(c_longlong, i))) < (count -% @as(usize, @bitCast(@as(c_longlong, @as(c_int, 1)))))) : (i += 1) {
            {
                var j: c_int = 0;
                while (@as(usize, @bitCast(@as(c_longlong, j))) < ((count -% @as(usize, @bitCast(@as(c_longlong, i)))) -% @as(usize, @bitCast(@as(c_longlong, @as(c_int, 1)))))) : (j += 1) {
                    if ((blk: {
                        const tmp = j;
...
pub const __seg_gs = @compileError("unable to translate macro: undefined identifier `__attribute__`"); // (no file):351:9
pub const __seg_fs = @compileError("unable to translate macro: undefined identifier `__attribute__`"); // (no file):352:9
...
pub const offsetof = @compileError("unable to translate macro: undefined identifier `__builtin_offsetof`"); // D:\install\zig-windows-x86_64\lib\include/stddef.h:116:9
...
pub const __btver2 = @as(c_int, 1);
pub const __btver2__ = @as(c_int, 1);
pub const __tune_btver2__ = @as(c_int, 1);
