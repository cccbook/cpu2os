...
void run(int* code, int size, SymbolTable* table) {
    ...
    while (pc < size) {
        switch (code[pc++]) {
            ...
            case OP_JMP: pc += (signed char)code[pc]; break;
            case OP_JIF: if (!stack[sp--]) pc += (signed char)code[pc]; else pc++; break;
            case OP_HALT: printf("執行結束，x = %d\n", table->symbols[0].value); return;
        }
    }
}
...
int main() {
    ...
    for (int i = 0; i < pos;) {
        switch (code[i]) {
            ...
            case OP_JMP: printf("JMP %d\n", (signed char)code[i+1]); i += 2; break;
