...
# restore mepc from scratch
        csrrw a0, mscratch, a0 #  交換 a0 和 mscratch (a0 = scratch 指標)
        
        lw      a1, 20(a0)     #  a1 = 儲存的 mepc (從 scratch[20] 載入)
        csrw    mepc, a1       #  將 mepc 設定為正確的中斷返回位址
        
        # 恢復被 sys_timer 儲存的 a1, a2, a3 暫存器
        # (這些是在 reg_load 之前被 C code 破壞的)
        lw      a1, 0(a0)      #  從 scratch[0] 恢復 a1
        lw      a2, 4(a0)      #  從 scratch[4] 恢復 a2
        lw      a3, 8(a0)      #  從 scratch[8] 恢復 a3

        csrrw a0, mscratch, a0 #  換回 a0 (恢復任務的 a0)
